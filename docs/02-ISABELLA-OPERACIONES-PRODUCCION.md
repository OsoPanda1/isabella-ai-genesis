...
**Estado:** `c70ea56` — `100%` implementación verificable (`545 passed`), `62%` despliegue hasta `Neon`/`Stripe`/`HSM` vivo — `81%` global honesto — `docs/evidence/c70ea56.json`

## 1. Gates de Producción (mismo SHA)
`INSTALL` `pnpm --frozen-lockfile` → `TYPECHECK 0` → `LINT 0` → `TEST 545` → `SECURITY` → `SBOM` → `DB` → `RLS` → `NCUA 50/100/250/500` → `BUILD 4.59s` → `VERCEL` → `HEALTH` → `SMOKE` → `BOOKPI` → `ROLLBACK` → `EVIDENCE` → `SAME_COMMIT`
...
`vite.config.ts` `nitro()` canónico + `esbuild jsxDev:false` + `generateBundle jsxDEV→jsx` (fix `V.jsxDEV`)
