...
| Inference policy (fail-closed prod) | `src/lib/inference-policy.ts`<br>`src/server-routes/api/isabella-voice.ts` | `test/unit/inference-authority.test.ts` | 503 maintenance en prod sin proveedor; nativo declarado solo dev. | real |
...
| Deploy Vercel con funciones (Nitro preset) | `vite.config.ts`<br>`vercel.json`<br>`src/routes/api/` | `test/integration/smoke.test.ts` | Build local genera .vercel/output/functions + config de rutas; wrappers estáticos bundlables. | real |
| Client env guard (sin secretos al bundle) | `scripts/check-client-env.mjs`<br>`package.json` | `test/unit/client-env.test.ts` | prebuild falla ante VITE_* secret-like; advierte no declaradas; 4 tests verdes. | real |
