...
export async function runLedgerAudit(
  ...
  if (!config().DATABASE_URL)
    throw new Error(
      "DATABASE_URL is required: financial audit never uses an in-memory repository",
    );
  ...
  if (!trialBalance.isBalanced) {
    ...
    recommendations.push(
      "Revisar entradas del período para identificar el desbalance.",
    );
  }
  ...
  if (!balanceSheet.isBalanced) {
    ...
    recommendations.push(
      "Verificar cuentas de resultado y cierre del período.",
    );
  }
  ...
  for (const entry of entries) {
    ...
    const totalDebits = lines.reduce(
      (sum: number, line: LedgerLine) => sum + line.debitCents,
      0,
    );
    const totalCredits = lines.reduce(
      (sum: number, line: LedgerLine) => sum + line.creditCents,
      0,
    );
  ...
  const pendingEntries = entries.filter(
    (entry: JournalEntry) => entry.status === "pending",
  );
  ...
  if (oldPending.length > 0)
    recommendations.push(
      "Revisar y asentar (post) o reversar las entradas pendientes antiguas.",
    );
  ...
  if (orphanLines > 0) {
    anomalies.push(
      `${orphanLines} líneas de diario sin entrada válida vinculada`,
    );
    recommendations.push(
      "Corregir referencialidad antes de cualquier cierre financiero.",
    );
  }

  if (trialBalance.accounts.length === 0)
    recommendations.push(
      "No hay actividad contable en el período; validar origen de datos.",
    );
  if (
    trialBalance.accounts.some((a) => Math.abs(a.netBalanceCents) > 1_000_000)
  )
