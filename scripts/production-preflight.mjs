...
if (existsSync(resolve(root, "src/server-routes/api/isabella.ts")))
  errors.push(
    "legacy duplicate src/server-routes/api/isabella.ts must not exist",
  );
const pkg = JSON.parse(readFileSync(resolve(root, "package.json"), "utf8"));
if (pkg.packageManager !== "pnpm@10.15.0")
  errors.push("packageManager must be pnpm@10.15.0");
if (pkg.engines?.node !== ">=22")
  errors.push("engines.node must be >=22 for deterministic production runtime");
for (const script of [
  "build",
  "start",
  "typecheck",
...
if (vercel.framework !== "tanstack-start")
  errors.push("vercel.framework must be tanstack-start");
...
if (/public-chat/i.test(server))
  errors.push(
    "server.ts must not expose the emergency public-chat demo gateway",
  );
...
const gateway = readFileSync(
  resolve(root, "src/lib/isabella-chat-gateway.ts"),
  "utf8",
);
for (const [pattern, label] of [
  ...
  if (!gateway.includes(pattern))
    errors.push(`canonical Isabella gateway missing ${label}`);
const migrationRunner = readFileSync(
  resolve(root, "scripts/db-migrate.mjs"),
  "utf8",
);
for (const [pattern, label] of [
  ...
  if (!migrationRunner.includes(pattern))
    errors.push(`Neon migration runner missing ${label}`);
if (errors.length) {
