...
const required = [
  ...
  "src/lib/isabella-chat-gateway.ts", "src/lib/api-contracts.ts", "src/lib/config.ts",
  "src/lib/principal-context.ts", "src/lib/production-authority.ts", "src/lib/runtime-integrity.ts",
...
for (const [pattern, label] of [["streamGenerateContent?alt=sse", "Gemini SSE streaming"], ["createSovereignPipeline", "sovereign governance"], ["checkRateLimitDistributed", "distributed rate limiting"], ['isKilled("inference")', "inference kill-switch"]])
...
if (errors.length) {
  ...
  else { console.error("Production preflight FAILED"); for (const error of errors) console.error(`- ${error}`); }
  process.exit(1);
}
