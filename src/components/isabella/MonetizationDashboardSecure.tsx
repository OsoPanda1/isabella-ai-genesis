...
export function MonetizationDashboardSecure({ initialTab }: { initialTab?: string | null }) {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 rounded-3xl border border-border/20 bg-background p-6 text-foreground shadow-xl">
    <header className="flex flex-col justify-between gap-4 border-b border-border/15 pb-5 md:flex-row md:items-center">
      ...
      <button disabled={loading} onClick={() => void refresh()} className="flex items-center gap-2 rounded-xl border border-border/20 px-3 py-2 font-mono text-xs disabled:opacity-50"><RefreshCw className={`size-4 ${loading ? "animate-spin" : ""}`} /> Actualizar</button>
    </header>
