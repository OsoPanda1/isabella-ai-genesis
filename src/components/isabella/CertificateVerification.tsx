...
import { ShieldCheck, Fingerprint, Lock, RotateCcw, HelpCircle, Check, AlertCircle } from "lucide-react";
...
export function CertificateVerification() {
  ...
  const [steps, setSteps] = useState<VerificationStep[]>([
    ...
    { label: "Verificando consistencia del Leaf Path Merkle contra el bloque raíz", status: "idle" },
    { label: "Descifrando y validando firma digital post-cuántica ML-DSA (FIPS 204)", status: "idle" },
    { label: "Verificando firma esférica resistente SLH-DSA (FIPS 205) de respaldo", status: "idle" },
    { label: "Validando veto de gobernanza ética y estado del Policy Gate de VIGIA", status: "idle" },
  ]);
  ...
  return (
    <div className="p-5 rounded-2xl bg-[#13151f] border border-border/10 space-y-4 text-muted-foreground text-xs" id="cert-verification-module">
      ...
      <p className="text-[11px] leading-relaxed">
        Verifique la autenticidad, integridad Merkle y firmas post-cuánticas de cualquier experimento procesado en la infraestructura qup v3.0 contra el Libro Mayor inmutable BookPI del Nodo Cero.
      </p>
          ...
          {steps.map((step, idx) => (
            <div key={idx} className="flex items-center justify-between gap-3">
              <span className={`text-left ${
                step.status === "success" ? "text-emerald-400" : step.status === "loading" ? "text-purple-400" : "text-muted-foreground"
              }`}>
                {idx + 1}. {step.label}
              </span>
              <span className="font-bold uppercase text-[9.5px]">
                {step.status === "idle" && <span className="text-muted-foreground/50">Espera</span>}
                {step.status === "loading" && <span className="text-purple-400 animate-pulse">Cargando</span>}
                {step.status === "success" && <span className="text-emerald-400 flex items-center gap-1"><Check className="size-3" /> OK</span>}
              </span>
            </div>
          ))}
        </div>
        ...
        <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/10 space-y-3 animate-rise font-mono text-[11.5px]">
          <div className="flex justify-between items-center pb-2 border-b border-purple-500/15">
            <span className="text-white font-bold uppercase flex items-center gap-1">
              <Fingerprint className="size-4 text-purple-400" /> Certificado de Autenticidad Cuántica
            </span>
          ...
          <div className="space-y-2 leading-relaxed">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-white truncate max-w-[200px] hover:text-clip" title="sha3_512_merkle_root_a8bc894cf9e31d">
          ...
          <div className="p-2.5 bg-black/45 rounded-lg border border-border/5 text-[10.5px] text-muted-foreground flex items-start gap-1.5 leading-tight">
            ...
            <div>
              Firmado por la clave privada HSM del Nodo Cero en Real del Monte, Hidalgo. Firma verificada mediante claves públicas pre-compartidas ML-DSA/SLH-DSA.
            </div>
          </div>
        </div>
      )}
