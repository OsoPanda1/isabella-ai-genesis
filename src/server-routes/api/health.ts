  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (path.endsWith("/live") || stage === "live" || url.searchParams.get("health") === "live") return liveness();
        if (path.endsWith("/ready") || stage === "ready" || url.searchParams.get("health") === "ready") return readiness();
...
async function liveness(): Promise<Response> {
  return json({ status: "alive", service: "isabella-ai-genesis", timestamp: new Date().toISOString() });
}

async function deepReadiness(): Promise<Response> {
  const base = await readiness();
  const body = (await base.json()) as { status: string; checks: Record<string, HealthCheck>; timestamp: string };
  try {
    const [{ config }, { resolveRuntimeMode }] = await Promise.all([import("@/lib/config"), import("@/lib/runtime-mode")]);
    const cfg = config();
    body.checks.runtime = { ok: true };
    body.checks.runtimeMode = { ok: Boolean(resolveRuntimeMode(cfg.ISABELLA_RUNTIME_MODE)) };
  ...
  const ok = body.status === "ready" && body.checks.bookpi?.ok === true && body.checks.runtime?.ok === true;
...
async function checkRepositoryHealth(kind: "repository" | "audit"): Promise<HealthCheck> {
  ...
  try {
    const { repositoryFactory } = await withTimeout(import("@/lib/persistence/repository-factory"), DEPENDENCY_TIMEOUT_MS);
    const repository = kind === "repository" ? repositoryFactory.getTenantRepository() : repositoryFactory.getAuditRepository();
    const result = await withTimeout(repository.health(), DEPENDENCY_TIMEOUT_MS);
    return { ok: result.ok, latencyMs: Number((performance.now() - started).toFixed(2)), ...(result.ok ? {} : { error: "repository_unhealthy" }) };
  } catch (error) {
    return {
      ok: false,
      latencyMs: Number((performance.now() - started).toFixed(2)),
      error: error instanceof Error && error.message === "dependency_timeout" ? "dependency_timeout" : `${kind}_unavailable`,
    };
  }
}
...
async function readiness(): Promise<Response> {
  ...
  try {
    const [{ config }, { isProductionLike, resolveRuntimeMode }] = await Promise.all([import("@/lib/config"), import("@/lib/runtime-mode")]);
    const cfg = config();
    ...
    checks.isabella_genesis = { ok: genesisConfigured, ...(genesisConfigured ? {} : { error: "genesis_service_unconfigured" }) };
  ...
  return json({ status: overallOk ? "ready" : "not_ready", checks, timestamp: new Date().toISOString() }, overallOk ? 200 : 503);
}
