      ...
      POST: withSovereignAuth("system", "execute", async (ctx, req) => {
        ...
        try {
          ...
          const r = await repo.append({
            ...
            userId: ctx.userId,
            operation: `MSR:${body.eventType ?? "GOVERNANCE_DECISION"}:${eventId}`,
            category: "other",
            cost: 0,
            tokens: 0,
            status: "settled",
        ...
        return new Response(
          JSON.stringify({
            ...
            data: {
              eventId,
              tenantId: ctx.tenantId,
              eventType: body.eventType ?? "GOVERNANCE_DECISION",
              actorId: ctx.userId,
              policyVersion: config().CROWN_CONSTITUTION_VERSION,
      ...
      GET: withSovereignAuth("system", "read", async (ctx) => {
        ...
        return new Response(
          JSON.stringify({
            ...
            timestamp: new Date().toISOString(),
            policyVersion: config().CROWN_CONSTITUTION_VERSION,
            implementation: "msr-v3",
            evidenceStatus: "E0",
            data: {
              info: "MSR append-only ledger, hash chain, WORM, snapshots — ver libro mayor BookPI",
            },
            error: null,
          }),
          { status: 200, headers },
