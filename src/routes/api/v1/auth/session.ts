    ...
    handlers: {
      POST: withSovereignAuth("system", "execute", async (ctx, req) => {
        ...
        return new Response(JSON.stringify({
          ...
          policyVersion: config().CROWN_CONSTITUTION_VERSION, implementation:"session-v1", evidenceStatus:"E0",
          data: { sessionId: `sess_${ctx.traceId.slice(0,8)}`, actorId: ctx.userId, tenantId: ctx.tenantId, role: ctx.role, scope: ctx.scope, expiresAt: new Date(Date.now()+3600*1000).toISOString(), bodyEcho: body },
          error: null,
        }), { status:200, headers });
      }),
      GET: withSovereignAuth("system", "read", async (ctx) => {
        ...
        return new Response(JSON.stringify({
          ...
          tenantId: ctx.tenantId, timestamp: new Date().toISOString(),
          policyVersion: config().CROWN_CONSTITUTION_VERSION, implementation:"session-v1", evidenceStatus:"E0",
          data: { actorId: ctx.userId, tenantId: ctx.tenantId, role: ctx.role, authenticated: true },
          error: null,
        }), { status:200, headers });
      }),
    },
