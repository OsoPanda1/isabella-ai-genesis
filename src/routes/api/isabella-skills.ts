  ...
  server: {
    handlers: {
      GET: withSovereignAuth("system", "read", async () =>
        json({ skills: listIsabellaSkills() }),
      ),
      ...
      POST: withSovereignAuth("system", "execute", async (context, request) => {
        ...
        const known = listIsabellaSkills().some(
          (skill) => skill.id === skillId,
        );
        if (!known)
          return json(
            { error: "SKILL_NOT_FOUND", skills: listIsabellaSkills() },
            404,
          );
          ...
          input,
          {
            ...
            intent:
              typeof payload.intent === "string"
                ...
                : "API skill execution",
            locale:
              typeof payload.locale === "string" ? payload.locale : "es-MX",
          },
        );
        const status = result.error
          ? result.error.code === "CROWN_POLICY_DENY"
            ? 403
            : 422
          : 200;
