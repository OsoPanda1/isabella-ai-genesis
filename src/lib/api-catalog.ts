...
export const CATALOG_ENTRIES: CatalogEntry[] = [
  ...
  route("health.readiness", "ops", "GET", "/api/health", "public-readiness contract", false, false, "Runtime liveness/readiness/deep-readiness checks with dependency timeouts.", "src/server-routes/api/health.ts"),
  route("observability.overview", "ops", "GET", "/api/observability", "server-auth + operational scope", false, true, "Runtime observability overview for traces, metrics, alerts, agents, sandboxes and cache.", "src/server-routes/api/observability.ts", "isabella.observability.v1"),
  route("health.live", "ops", "GET", "/api/health/live", "public-liveness contract", false, false, "Low-cost process liveness probe.", "src/routes/api/health/live.ts"),
  route("health.ready", "ops", "GET", "/api/health/ready", "public-readiness contract", false, false, "Readiness probe for durable dependencies and configured runtime authority.", "src/routes/api/health/ready.ts"),
  route("health.deep", "ops", "GET", "/api/health/deep", "public-readiness contract", false, false, "Deep readiness probe including runtime mode and BookPI configuration.", "src/routes/api/health/deep.ts"),
  route("catalog.list", "ops", "GET", "/api/catalog", "server-auth + tenant + scope", false, true, "Canonical API capability catalog derived from executable route metadata.", "src/server-routes/api/catalog.ts"),
  route("billing.gateway", "billing", "GET", "/api/billing", "server-auth + tenant + billing policy", false, true, "Durable billing, ledger and marketplace operations exposed by the billing handler.", "src/server-routes/api/billing.ts"),
  route("security.gateway", "identity", "POST", "/api/security", "server-auth + policy", true, true, "Security operations routed through the server-side security handler.", "src/server-routes/api/security.ts"),
  route("db.gateway", "identity", "GET", "/api/db", "server-auth + tenant", false, true, "Durable persistence diagnostics and governed database operations.", "src/server-routes/api/db.ts"),
