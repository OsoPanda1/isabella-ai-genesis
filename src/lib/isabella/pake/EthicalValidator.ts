...
export class EthicalValidator {
   ...
   */
  public static auditContent(content: string): {
    valid: boolean;
    score: number;
    flags: string[];
    hash: string;
    ...
    if (
      lowerContent.includes("ocultar") ||
      lowerContent.includes("secreto") ||
      lowerContent.includes("engaño")
    ) {
    ...
    if (
      ...
      lowerContent.includes("discriminación") ||
      lowerContent.includes("excluir")
    ) {
