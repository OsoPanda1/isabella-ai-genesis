...
export function convergeEvolvedSkillResults(
  ...
  if (observations.length < 2) {
    // Single observation synthetic baseline to maintain mathematical consistency
    const first = observations[0]!;
    const baselineObs: KnowledgeObservation = {
      ...first,
      teacherId: `${first.teacherId}_validator`,
      confidence: Math.max(0.1, first.confidence - 0.05),
      provenanceHash: hash({ baseline: true, parentHash: first.provenanceHash }),
    };
    return convergeKnowledge([first, baselineObs]);
  }
