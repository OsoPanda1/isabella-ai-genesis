...
export type NativeSkillDomain =
  ...
  | "documentation" | "presentation" | "visual_media" | "code_engineering"
  | "security" | "financial" | "legal" | "healthcare" | "life_sciences"
  | "education" | "commerce" | "research" | "devops" | "automation"
  | "mcp_integrations" | "general_reasoning";
export type NativeSkillExecutionStatus = "SUCCESS" | "PARTIAL" | "BLOCKED" | "FAILED";
export interface NativeSkillSource { id: string; repository: string; family: string; capabilityDomains: readonly NativeSkillDomain[]; native: true; externalSideEffectsRequireAdapter: true; }
export interface NativeSkillRequest { skillId: string; source?: string; task?: string; input?: Record<string, unknown>; locale?: string; actorId?: string; tenantId?: string; requestId?: string; }
export interface NativeSkillExecution {
  requestId: string; skillId: string; source: string; domain: NativeSkillDomain; status: NativeSkillExecutionStatus;
  native: true; externallyExecutable: boolean; summary: string; plan: readonly string[]; features: readonly number[];
  risk: { score: number; labels: readonly string[] }; semanticVector: readonly number[];
  evidence: readonly { id: string; type: "native-analysis" | "input-provenance"; hash: string; source: string }[];
  provenanceHash: string; warnings: readonly string[]; missingAdapters: readonly string[];
}
...
const DOMAIN_RULES: ReadonlyArray<{ domain: NativeSkillDomain; terms: readonly string[] }> = [
  ...
  { domain: "agent_development", terms: ["agent", "claude", "orchestration", "loop", "tool", "sdk"] },
...
const DOMAIN_PLANS: Record<NativeSkillDomain, readonly string[]> = {
  ...
  agent_development: ["Definir objetivo", "Separar modelo, herramientas, memoria y política", "Establecer límites", "Diseñar pruebas"],
  skill_engineering: ["Formalizar intención", "Definir contrato", "Construir evaluación", "Medir regresiones"],
  knowledge_work: ["Extraer entidades", "Separar hechos de inferencias", "Sintetizar", "Conservar provenance"],
  documentation: ["Identificar audiencia", "Estructurar conceptos", "Especificar ejemplos", "Verificar código y docs"],
  presentation: ["Definir narrativa", "Diseñar estructura", "Aplicar composición", "Validar legibilidad"],
  visual_media: ["Definir formato", "Diseñar composición", "Preparar recursos", "Validar derechos y accesibilidad"],
  code_engineering: ["Inspeccionar contratos", "Modelar cambio reversible", "Implementar con pruebas", "Ejecutar gates"],
  security: ["Clasificar amenaza", "Identificar autoridad", "Aplicar fail-closed", "Probar abuso y aislamiento"],
  financial: ["Identificar instrumento", "Normalizar datos", "Separar observación y supuesto", "Calcular escenarios con provenance"],
  legal: ["Identificar jurisdicción", "Separar fuente e interpretación", "Construir documento", "Marcar revisión profesional"],
  healthcare: ["Identificar contexto y estándar", "Normalizar entidades", "Aplicar privacidad", "Marcar decisión clínica"],
  life_sciences: ["Definir pregunta", "Establecer datos y controles", "Analizar reproduciblemente", "Registrar límites"],
  education: ["Definir objetivo", "Adaptar dificultad", "Construir actividad", "Verificar inclusión"],
  commerce: ["Definir operación", "Validar identidad y política", "Separar cálculo de autoridad financiera", "Registrar idempotencia"],
  research: ["Definir pregunta", "Separar evidencia primaria y secundaria", "Comparar hipótesis", "Registrar incertidumbre"],
  devops: ["Inspeccionar artefactos", "Validar entorno", "Ejecutar gates", "Preparar observabilidad y rollback"],
...
function detectDomain(skillId: string, task: string): NativeSkillDomain {
  ...
  let best: { domain: NativeSkillDomain; score: number } = { domain: "general_reasoning", score: 0 };
  for (const rule of DOMAIN_RULES) {
    const score = rule.terms.reduce((sum, term) => sum + (haystack.includes(normalize(term)) ? 1 : 0), 0);
...
function inferSource(skillId: string, explicitSource?: string): NativeSkillSource {
  ...
  const exact = requested ? NATIVE_SKILL_SOURCES.find((s) => normalize(s.id) === requested || normalize(s.repository) === requested) : undefined;
  if (exact) return exact;
  return NATIVE_SKILL_SOURCES.find((s) => normalize(skillId).includes(normalize(s.family).replaceAll("-", " "))) ?? {
    id: "native-isabella", repository: "OsoPanda1/isabella-ai-genesis", family: "native-fusion",
    capabilityDomains: ["general_reasoning"], native: true, externalSideEffectsRequireAdapter: true,
  };
}
...
export function executeNativeSkill(request: NativeSkillRequest): NativeSkillExecution {
  ...
  const provenancePayload = { requestId, skillId, source: source.repository, domain, task, input, riskModel: risk.modelId, plan };
  const provenanceHash = stableHash(provenancePayload);
  const warnings = external ? ["La ejecución nativa no simula efectos externos; requiere un adaptador autorizado."] : [];
  const missingAdapters = external ? ["external-side-effect-adapter"] : [];
  return {
    requestId, skillId, source: source.repository, domain,
    status: external ? "PARTIAL" : "SUCCESS", native: true, externallyExecutable: false,
    summary: external ? `${skillId}: capacidad nativa resuelta; el efecto externo está bloqueado hasta autorizar un adaptador.` : `${skillId}: capacidad resuelta por Isabella Native ML para análisis, planificación y trazabilidad.`,
    plan, features: risk.features, risk: { score: risk.riskScore, labels: risk.labels }, semanticVector,
    evidence: [
      { id: `evi_${randomUUID().slice(0, 8)}`, type: "native-analysis", hash: stableHash({ skillId, domain, risk, task }), source: "Isabella Native ML / NCUA" },
      { id: `evi_${randomUUID().slice(0, 8)}`, type: "input-provenance", hash: stableHash(input), source: source.repository },
    ],
