...
import type { DatasetIdentity, ModelIdentity, NativeMLHooks, PredictionResult, TrainingResult, MLTask, ModelArtifact } from "./types";
...
function validateArtifact(model: ModelIdentity, artifact: ModelArtifact): void {
  if (!Number.isFinite(artifact.bias) || artifact.weights.length === 0 || artifact.weights.some((v) => !Number.isFinite(v))) throw new Error("invalid_model_artifact");
  const artifactHash = hash({ weights: artifact.weights, bias: artifact.bias });
  if (artifactHash !== model.modelHash || artifact.artifactHash !== artifactHash) throw new Error("model_artifact_hash_mismatch");
}
...
export async function trainBinaryClassifier(input: BinaryDataset, ownerId: string, territoryId: string, hooks: NativeMLHooks = {}): Promise<TrainingResult> {
  ...
  const weights = Array.from({ length: width }, () => 0); let bias = 0; const lr = 0.05;
  for (let epoch = 0; epoch < 40; epoch++) for (let i = 0; i < input.features.length; i++) {
    const row = input.features[i]!; const y = input.labels[i]!; const p = sigmoid(row.reduce((sum, x, j) => sum + x * weights[j]!, bias)); const error = p - y;
    for (let j = 0; j < width; j++) weights[j] = weights[j]! - lr * error * row[j]!; bias -= lr * error;
  }
  let correct = 0; let loss = 0;
  ...
  const model: ModelIdentity = { modelId, version: "0.1.0", territoryId, ownerId, task: "classification", algorithm: "deterministic-logistic-sgd", datasetIds: [input.dataset.datasetId], modelHash: artifactHash, createdAt: new Date().toISOString(), approvalStatus: "PENDING_REVIEW" };
  const artifact: ModelArtifact = { weights: [...weights], bias, artifactHash };
  const trainingHash = hash({ dataset: input.dataset, weights, bias }); const provenanceId = `prov_${hash({ dataset: input.dataset, trainingHash }).slice(0, 24)}`;
  await hooks.audit?.("native_ml.training_completed", { modelId, trainingHash, provenanceId });
  return { model, metrics: { accuracy: correct / input.features.length, logLoss: loss / input.features.length }, trainingHash, provenanceId, approvalRequired: true, artifact };
}
...
export async function predictBinary(model: ModelIdentity, weights: number[], bias: number, features: number[][], hooks: NativeMLHooks = {}): Promise<PredictionResult<number>> {
  ...
  const artifact: ModelArtifact = { weights, bias, artifactHash: hash({ weights, bias }) }; validateArtifact(model, artifact);
  ...
  const predictions = probabilities.map((p) => p >= 0.5 ? 1 : 0); const confidence = probabilities.map((p) => Math.max(p, 1 - p));
  let minConfidence = 1; for (const value of confidence) if (value < minConfidence) minConfidence = value;
  const auditId = await hooks.audit?.("native_ml.prediction", { modelId: model.modelId, artifactHash: artifact.artifactHash, count: predictions.length });
  return { model, predictions, confidence, explanation: { method: "linear_feature_contribution", featureCount: weights.length }, riskScore: Math.max(0, 1 - minConfidence), requiresReview: decision?.decision === "REVIEW", degraded: false, ...(auditId ? { auditId } : {}) };
}
