...
function mac(value: string): string { return createHmac("sha256", secret()).update(value).digest("base64url"); }
function cleanup(now: number): void { for (const [jti, expiresAt] of consumed) if (expiresAt <= now) consumed.delete(jti); }
...
export function issueCapabilityToken(input: Omit<CapabilityTokenClaims, "jti" | "nonce" | "expiresAt"> & { ttlMs?: number }): string {
  ...
  const claims: CapabilityTokenClaims = { tool: input.tool, actorId: input.actorId, tenantId: input.tenantId, expiresAt: now + ttl, jti: randomBytes(18).toString("base64url"), nonce: randomBytes(18).toString("base64url") };
...
export function consumeCapabilityToken(token: string, expected: { tool: string; actorId: string; tenantId: string }): CapabilityTokenClaims {
  ...
  try { claims = JSON.parse(Buffer.from(payload, "base64url").toString("utf8")) as CapabilityTokenClaims; } catch { throw new Error("capability_token_invalid"); }
  if (!claims.jti || !claims.nonce || claims.tool !== expected.tool || claims.actorId !== expected.actorId || claims.tenantId !== expected.tenantId) throw new Error("capability_token_scope_mismatch");
