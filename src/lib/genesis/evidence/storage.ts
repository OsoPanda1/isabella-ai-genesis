...
import { createHash, randomBytes, sign, verify, generateKeyPairSync, KeyObject } from "node:crypto";
import { Evidence } from "../schemas/evidence.schema";
import { Manifest, createEmptyManifest, ManifestContext, ManifestIntegrity } from "../schemas/manifest.schema";
...
export class EvidenceStorage {
  ...
  async listEvidences(): Promise<string[]> {
    ...
    return fs.readdirSync(evidenceDir).filter(name => {
      ...
      return fs.statSync(itemPath).isDirectory() && fs.existsSync(path.join(itemPath, "evidence.json"));
    });
  }
  ...
  async saveManifest(manifest: Manifest): Promise<string> {
    ...
    if (this.signingKey) {
      ...
      fs.writeFileSync(pubKeyPath, this.signingKey.export({ format: "pem", type: "public" }).toString(), "utf8");
    }
  ...
  generateSigningKeyPair(): { privateKey: KeyObject; publicKey: string } {
    ...
    return { privateKey, publicKey: publicKey.export({ format: "pem", type: "public" }).toString() };
  }
