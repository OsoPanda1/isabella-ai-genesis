...
export interface FinancialCriticalFinding {
  type: "NON_ATOMIC_FINANCIAL_MUTATION" | "DOUBLE_SPEND_VULNERABILITY" | "IDEMPOTENCY_MISSING" | "WEBHOOK_RACE_CONDITION" | "BALANCE_NEGATIVE_ALLOWED" | "PAYOUT_WITHOUT_VERIFICATION" | "REFUND_NOT_ATOMIC" | "LEDGER_INCONSISTENT";
  location: string;
  description: string;
  risk: "CRITICAL" | "HIGH";
  example: string;
...
export class FinancialScanner {
  ...
  private runAtomicityTests(operations: FinancialOperation[], allContent: Map<string, string>): AtomicityTest[] {
    return [
      {
  ...
  private runIdempotencyTests(operations: FinancialOperation[], allContent: Map<string, string>): IdempotencyTest[] {
    const webhookOps = operations.filter(o => o.type === "webhook_handling");
    const mechanism = webhookOps.length > 0 && webhookOps.some(o => o.idempotent) 
      ? "event_id" 
      : "idempotency_key";
  ...
  private findCriticalIssues(operations: FinancialOperation[], allContent: Map<string, string>): FinancialCriticalFinding[] {
    const findings: FinancialCriticalFinding[] = [];
    
    const nonAtomic = operations.filter(o => !o.atomic && (o.type === "debit" || o.type === "credit" || o.type === "transfer"));
    for (const op of nonAtomic) {
      findings.push({
        ...
        remediation: "Implementar transacciones atómicas con BEGIN/COMMIT y validación de balance dentro de la transacción",
      });
    }
    
    const missingIdempotency = operations.filter(o => !o.idempotent && o.type === "webhook_handling");
    ...
    const missingBalanceCheck = operations.filter(o => !o.balanceCheck && (o.type === "debit" || o.type === "transfer"));
    ...
    const missingLedger = operations.filter(o => !o.ledgerIntegrated && (o.type === "debit" || o.type === "credit" || o.type === "transfer" || o.type === "refund"));
