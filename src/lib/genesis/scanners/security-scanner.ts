...
export interface SecretFinding {
  id: string;
  type: "API_KEY" | "PRIVATE_KEY" | "JWT_SECRET" | "DB_CREDENTIAL" | "CLOUD_CREDENTIAL" | "STRIPE_SECRET" | "SIGNING_KEY" | "ENCRYPTION_KEY" | "OAUTH_SECRET" | "WEBHOOK_SECRET" | "HIGH_ENTROPY";
  file: string;
  line: number;
  column: number;
  fingerprint: string;
...
export interface ConfigIssueFinding {
  id: string;
  type: "CSP_UNSAFE_INLINE" | "CSP_REPORT_ONLY" | "HSTS_MISSING" | "X_FRAME_OPTIONS_MISSING" | "SECURE_COOKIES_MISSING" | "CORS_WILDCARD" | "DEBUG_ENABLED" | "WEAK_CRYPTO";
  file: string;
  line: number;
  description: string;
  severity: "CRITICAL" | "HIGH" | "MEDIUM" | "LOW";
...
const SECRET_PATTERNS = [
  { type: "API_KEY" as const, pattern: /(?:api[_-]?key|apikey)\s*[:=]\s*['"]([a-zA-Z0-9_-]{20,})['"]/gi, entropy: false },
  { type: "PRIVATE_KEY" as const, pattern: /-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/g, entropy: false },
  { type: "JWT_SECRET" as const, pattern: /(?:jwt[_-]?secret|jwtsecret)\s*[:=]\s*['"]([^'"]{16,})['"]/gi, entropy: false },
  { type: "DB_CREDENTIAL" as const, pattern: /(?:database[_-]?url|db[_-]?url)\s*[:=]\s*['"]([^'"]{10,})['"]/gi, entropy: false },
  { type: "STRIPE_SECRET" as const, pattern: /sk_(?:live|test)_[a-zA-Z0-9]{24,}/g, entropy: false },
  { type: "SIGNING_KEY" as const, pattern: /(?:signing[_-]?key|signingkey)\s*[:=]\s*['"]([^'"]{16,})['"]/gi, entropy: false },
  { type: "ENCRYPTION_KEY" as const, pattern: /(?:encryption[_-]?key|master[_-]?key)\s*[:=]\s*['"]([^'"]{16,})['"]/gi, entropy: false },
  { type: "OAUTH_SECRET" as const, pattern: /(?:oauth[_-]?secret|client[_-]?secret)\s*[:=]\s*['"]([^'"]{16,})['"]/gi, entropy: false },
  { type: "WEBHOOK_SECRET" as const, pattern: /(?:webhook[_-]?secret|whsec)_[a-zA-Z0-9]{16,}/g, entropy: false },
];
...
const CONFIG_PATTERNS = [
  { pattern: /Content-Security-Policy.*unsafe-inline/g, type: "CSP_UNSAFE_INLINE" as const, severity: "HIGH" },
  { pattern: /Content-Security-Policy-Report-Only/g, type: "CSP_REPORT_ONLY" as const, severity: "MEDIUM" },
  { pattern: /Strict-Transport-Security/g, type: "HSTS_MISSING" as const, severity: "MEDIUM", negative: true },
  { pattern: /X-Frame-Options/g, type: "X_FRAME_OPTIONS_MISSING" as const, severity: "LOW", negative: true },
  { pattern: /cookie.*secure.*false/g, type: "SECURE_COOKIES_MISSING" as const, severity: "MEDIUM" },
  ...
  { pattern: /crypto\.createHash\s*\(\s*['"]md5['"]\s*\)/g, type: "WEAK_CRYPTO" as const, severity: "HIGH" },
  { pattern: /crypto\.createHash\s*\(\s*['"]sha1['"]\s*\)/g, type: "WEAK_CRYPTO" as const, severity: "HIGH" },
];
...
export class SecurityScanner {
  ...
  private scanVulnerabilities(filePath: string, content: string, lines: string[]): VulnerabilityFinding[] {
  ...
  private scanConfigIssues(filePath: string, content: string, lines: string[]): ConfigIssueFinding[] {
  ...
  private collectFiles(dir: string): string[] {
    const files: string[] = [];
    const includePatterns = ["**/*.ts", "**/*.tsx", "**/*.js", "**/*.jsx", "**/*.json", "**/*.yaml", "**/*.yml", "**/*.md", "**/*.html"];
    const excludePatterns = ["**/node_modules/**", "**/dist/**", "**/build/**", "**/.git/**", "**/coverage/**", "**/genesis/**", "**/*.test.ts", "**/*.spec.ts", "**/*.snap"];
