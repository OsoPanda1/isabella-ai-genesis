...
export function calculateCVSS(finding: Finding): { baseScore: number; vectorString: string } {
  const { exploitability, impact } = finding;
  
  // CVSS 3.1 Base Score Calculation
  const AV = exploitability.attackVector === "NETWORK" ? 0.85 : 
             exploitability.attackVector === "ADJACENT" ? 0.62 :
             exploitability.attackVector === "LOCAL" ? 0.55 : 0.2;
  const AC = exploitability.attackComplexity === "LOW" ? 0.77 : 0.44;
  const PR = exploitability.scope === "CHANGED" ? 
             (exploitability.privilegesRequired === "NONE" ? 0.85 :
              exploitability.privilegesRequired === "LOW" ? 0.68 : 0.5) :
             (exploitability.privilegesRequired === "NONE" ? 0.85 :
              exploitability.privilegesRequired === "LOW" ? 0.62 : 0.27);
...
export function calculatePriority(finding: Finding): { score: number; rank: number; shouldBlockRelease: boolean; slaHours: number } {
  ...
  const blastRadiusBonus = finding.blastRadius === "SYSTEM" ? 15 : finding.blastRadius === "TENANT" ? 10 : finding.blastRadius === "USER" ? 5 : 0;
  
  const score = Math.min(Math.round(baseScore * categoryMultiplier + exploitabilityBonus + blastRadiusBonus), 100);
  const shouldBlockRelease = finding.severity === "CRITICAL" || (finding.severity === "HIGH" && finding.category === "FINANCIAL");
  const slaHours = finding.severity === "CRITICAL" ? 4 : finding.severity === "HIGH" ? 24 : finding.severity === "MEDIUM" ? 72 : 168;
