...
export class FindingGenerator {
  ...
  generateFromClaimEvaluation(claim: Claim, verificationResult: {
    status: string;
      ...
      for (const gap of verificationResult.gaps) {
        ...
        findings.push(this.createFinding({
          ...
          remediation: {
            ...
            effort: "MEDIUM",
            verificationSteps: ["Verificar evidencia", "Ejecutar tests", "Actualizar documentación"],
          },
        }));
      }
    }
    ...
    if (verificationResult.confidenceScore < 0.7) {
      findings.push(this.createFinding({
        ...
        remediation: {
          ...
          effort: "HIGH",
          verificationSteps: ["Revisar evidencia existente", "Generar evidencia faltante", "Re-ejecutar tests"],
        },
      }));
    }
  ...
  generateFromEvidenceQuality(evidence: Evidence, quality: {
    ...
    if (!quality.sourceVerification) {
      findings.push(this.createFinding({
        ...
        severity: "HIGH",
        category: "AUDIT",
        evidence: [{ type: evidence.type, hash: evidence.content.hash, description: evidence.metadata.collectedAt }],
        remediation: {
  ...
  generateFromContradiction(contradiction: {
    ...
    return this.createFinding({
      ...
      remediation: {
        ...
        effort: "HIGH",
        verificationSteps: ["Identificar fuente de verdad", "Corregir implementación o documentación", "Re-verificar"],
      },
    });
  }

  generateFromMissingEvidence(claim: Claim, missingTypes: string[]): Finding[] {
    return missingTypes.map(type => this.createFinding({
      ...
      remediation: {
        ...
        effort: "HIGH",
        verificationSteps: [`Ejecutar tests para ${type}`, "Verificar resultados", "Registrar evidencia"],
      },
    }));
  }
  ...
  private createFinding(params: {
    ...
    remediation?: { description: string; effort: "LOW" | "MEDIUM" | "HIGH"; owner?: string; dueDate?: string; compensatingControl?: string; verificationSteps?: string[] };
  }): Finding {
    const baseFinding = {
      ...
      impact: { confidentiality: 0, integrity: 0, availability: 0, financial: 0, reputational: 0, compliance: 0 },
      exploitability: { attackVector: "NETWORK", attackComplexity: "LOW", privilegesRequired: "NONE", userInteraction: "NONE", scope: "UNCHANGED" },
      blastRadius: "COMPONENT" as const,
      priority: { score: 0, rank: 0, shouldBlockRelease: false, slaHours: 24 },
      remediation: params.remediation ?? { description: "Remediation needed", effort: "MEDIUM", verificationSteps: [] },
