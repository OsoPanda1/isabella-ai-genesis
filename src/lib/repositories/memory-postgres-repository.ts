...
function getPool(): Pool {
  if (!pool) {
    ...
    pool = new Pool({ connectionString: dsn, max: 10, connectionTimeoutMillis: 10_000, idleTimeoutMillis: 30_000, statement_timeout: 15_000 });
  }
  return pool;
}
...
function contentHash(input: { id: string; tenantId: string; content: string; source: MemorySource; scope: MemoryScope; sensitivity: MemorySensitivity }): string {
  return sha256(`${input.id}|${input.tenantId}|${input.content}|${input.source}|${input.scope}|${input.sensitivity}`);
}

function chainHash(previous: string, contentDigest: string): string { return sha256(`${previous}|${contentDigest}`); }
...
function mapScope(value: unknown): MemoryScope {
  ...
  if (scope === "turn" || scope === "session" || scope === "project" || scope === "territorial" || scope === "historical") return scope;
...
function mapSensitivity(value: unknown): MemorySensitivity {
  ...
  if (sensitivity === "public" || sensitivity === "internal" || sensitivity === "personal" || sensitivity === "restricted") return sensitivity;
...
function validateInput(input: Parameters<MemoryRepository["add"]>[0]): string | null {
  ...
  if ((input.sensitivity === "personal" || input.sensitivity === "restricted") && !input.ownerId) return "ownerId requerido para memoria personal/restringida.";
  return null;
}
  ...
  return {
    async add(input: Parameters<MemoryRepository["add"]>[0]) {
      ...
      try {
        ...
        return { success: false as const, error: cause instanceof Error ? cause.message : "memory_insert_failed" };
      } finally {
        client.release();
      }
    },
