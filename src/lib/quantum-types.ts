...
export const ZneConfigurationSchema = z.object({
  level: z
    .literal(3)
    .describe(
      "QUP v3.0 requires strictly ZNE level 3 for sovereign operations",
    ),
  extrapolationMethod: z.enum(["linear", "richardson", "exponential"]),
  scaleFactors: z
    .array(z.number())
    .min(3, "At least 3 scale factors required for ZNE Level 3"),
});
...
export const PecConfigurationSchema = z.object({
  ...
  learningPhaseFidelity: z
    .number()
    .min(0.99, "Minimum 99% fidelity required in learning phase"),
});
...
export const SovereignQuantumRuntimeSchema = z.object({
  ...
  maxExecutionTimeMs: z
    ...
    .max(
      60000,
      "Max execution time cannot exceed 60 seconds for sovereign nodes",
    ),
...
export type SovereignQuantumRuntime = z.infer<
  typeof SovereignQuantumRuntimeSchema
>;
