...
export function calculatePhoenixAllocation(
  netProfitCents: number,
): PhoenixAllocation {
  ...
  const infrastructureCents =
    Math.floor(netProfitCents / 10) * 3 +
    Math.floor(((netProfitCents % 10) * 3) / 10);
  const creatorPartnersCents =
    netProfitCents - phoenixFundCents - infrastructureCents;
  return {
    netProfitCents,
    phoenixFundCents,
    infrastructureCents,
