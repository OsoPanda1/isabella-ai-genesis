...
export class InMemoryLearningStore implements LearningStore {
  ...
  list(): LearningMemory[] {
    return [...this.memories.values()].map((memory) => ({ ...memory, concepts: [...memory.concepts], procedures: [...memory.procedures], preferences: [...memory.preferences] }));
  }

  get(id: string): LearningMemory | undefined {
    const memory = this.memories.get(id);
    return memory ? { ...memory, concepts: [...memory.concepts], procedures: [...memory.procedures], preferences: [...memory.preferences] } : undefined;
  }
  ...
  updateCompetence(skillId: string, outcome: LearningExample["outcome"], quality: number): Competence {
    ...
    if (!existing) {
      const created = { skillId, score: clamp(0.5 + signal * 0.25, 0, 1), observations: 1, successes: success, failures: failure, lastUpdatedAt: now };
...
export class IsabellaLearningEngine {
  ...
  ingest(raw: unknown): LearningResult {
    ...
    if (!parsed.success) return { accepted: false, reason: parsed.error.issues.map((issue) => issue.message).join("; "), competence: [], concepts: [] };
    const example = sanitizeExample(parsed.data);
    if (!example.consent && example.mode !== "supervised") {
      return { accepted: false, reason: "Explicit consent is required for durable non-supervised learning.", competence: [], concepts: [] };
    }
    if (containsPromptInjection(example.input) || (example.target && containsPromptInjection(example.target))) {
      return { accepted: false, reason: "Learning sample rejected by prompt-injection guard.", competence: [], concepts: [] };
    }

    const concepts = extractConcepts(example.input, example.target, example.context);
    const procedures = example.mode === "procedural" ? extractProcedures(example.input, example.target) : [];
    const preferences = example.mode === "preference" ? extractPreferences(example.input, example.target) : [];
    ...
    const competence = example.skillIds.map((skillId) => this.store.updateCompetence(skillId, example.outcome, example.quality));
  ...
  retrieve(query: string, limit = 8): LearningMemory[] {
    ...
    return this.store.list()
      ...
      .sort((a, b) => b.score - a.score || b.memory.lastReinforcedAt.localeCompare(a.memory.lastReinforcedAt))
      .slice(0, Math.max(1, Math.min(limit, 50)))
      .map((item) => item.memory);
  }

  evaluate(query: string): { score: number; supportingMemories: string[]; concepts: string[] } {
    const concepts = extractConcepts(query);
    const memories = this.retrieve(query, 12);
    const score = clamp(memories.reduce((sum, memory) => sum + memory.quality, 0) / Math.max(memories.length, 1), 0, 1);
    return { score, supportingMemories: memories.map((memory) => memory.id), concepts };
  }
...
export function createIsabellaLearningEngine(store?: InMemoryLearningStore): IsabellaLearningEngine {
...
function extractConcepts(...values: Array<string | Record<string, unknown> | undefined>): string[] {
  const text = values.filter((value) => value !== undefined).map((value) => typeof value === "string" ? value : JSON.stringify(value)).join(" ").toLowerCase();
  const words = text.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").match(/[a-záéíóúñü]{4,}/gi) ?? [];
  const stop = new Set(["para", "como", "desde", "este", "esta", "that", "with", "from", "have", "will", "then", "when", "what", "into", "sobre", "entre"]);
  return [...new Set(words.filter((word) => !stop.has(word)).map((word) => word.slice(0, 48)))].slice(0, 32);
}

function extractProcedures(input?: string, target?: string): string[] {
  const text = [input, target].filter(Boolean).join(" ");
  return (text.match(/(?:\d+[.)]|primero|despues|después|luego|finalmente)[^.!?]{3,160}/gi) ?? []).slice(0, 12).map(sanitizeText);
}

function extractPreferences(input?: string, target?: string): string[] {
  const text = [input, target].filter(Boolean).join(" ");
  return (text.match(/(?:prefiero|preferimos|evita|evitar|me gusta|no me gusta|should|prefer|avoid)[^.!?]{2,140}/gi) ?? []).slice(0, 12).map(sanitizeText);
}

function stableSignature(example: LearningExample, concepts: string[]): string {
  return createHash("sha3-512").update(JSON.stringify({ mode: example.mode, input: example.input, target: example.target ?? "", negative: example.negative ?? "", concepts })).digest("hex");
}

function relevance(memory: LearningMemory, concepts: string[]): number {
  if (concepts.length === 0) return 0;
  const overlap = concepts.filter((concept) => memory.concepts.includes(concept)).length / concepts.length;
