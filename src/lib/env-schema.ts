...
const emptyToUndefined = (schema: z.ZodTypeAny) => z.preprocess((val) => {
...
const bool = (def: boolean) => z.preprocess((val) => {
...
export const envSchema = z.object({
  ...
  DATABASE_URL: optionalString(), DATABASE_DIRECT_URL: optionalString(), INTERNAL_ORIGIN: optionalUrl(),
  SUPABASE_URL: optionalUrl(), SUPABASE_ANON_KEY: optionalString(), SUPABASE_SERVICE_ROLE_KEY: optionalString(), SUPABASE_JWT_SECRET: optionalString(),
  AUTH_JWT_SECRET: optionalMinString(16), SESSION_SECRET: optionalMinString(32), AUTH_ISSUER: optionalUrl(), AUTH_AUDIENCE: z.string().default("isabella"),
  AUTH_ACCESS_TOKEN_TTL: coercedInt(3600), AUTH_REFRESH_TOKEN_TTL: coercedInt(604800), OIDC_JWKS_URL: optionalUrl(), JWKS_CACHE_TTL: coercedInt(3600),
  ...
  ALLOW_GUEST_CHAT: z.preprocess((val) => typeof val === "string" ? val.trim().toLowerCase() : undefined, z.enum(["true", "false"]).default("false")).transform((v) => v === "true"),
  PROVISION_OWNER_TOKEN: optionalString(),
  ENCRYPTION_MASTER_KEY: optionalMinString(32), ENCRYPTION_ALGORITHM: z.string().default("aes-256-gcm"),
  CROWN_CONSTITUTION_VERSION: z.string().min(1).default("v4.2.0-sovereign"), CROWN_POLICY_SIGNING_KEY: optionalString(), AEGIS_AUDIT_SECRET: optionalMinString(32),
  CROWN_ENFORCEMENT_MODE: z.enum(["enforce", "dry-run"]).default("enforce"),
  BOOKPI_SIGNATURE_ALGORITHM: z.enum(["ML-DSA-87", "ECDSA-P384", "RSA-SHA256"]).default("ECDSA-P384"), BOOKPI_SIGNING_KEY: optionalMinString(32),
  STRIPE_SECRET_KEY: optionalMinString(16), STRIPE_WEBHOOK_SECRET: optionalMinString(16),
  QUP_ZNE_LEVEL: coercedInt(3), QUP_PEC_ENABLED: bool(true), QUP_QEC_DECODER: z.enum(["mwpm", "uf", "tensor-network", "neural-network"]).default("tensor-network"), QUP_STRICT_ISOLATION: bool(true),
  ...
  RATE_LIMIT_DEFAULT_PER_MINUTE: coercedInt(120), RATE_LIMIT_INFERENCE_PER_MINUTE: coercedInt(40), RATE_LIMIT_VOICE_PER_MINUTE: coercedInt(20),
  GEMINI_API_KEY: optionalString(), LLM_DEFAULT_MODEL: z.string().default("google/gemini-3.8-flash"), LLM_VOICE_MODEL: z.string().default("openai/gpt-4o-mini-tts"), VOICE_API_URL: optionalUrl(), LLM_UPSTREAM_TIMEOUT_MS: coercedInt(8500),
  OTEL_EXPORTER_OTLP_ENDPOINT: optionalUrl(), OTEL_SERVICE_NAME: z.string().default("isabella-ai"),
  ISABELLA_FEATURE_FLAGS: z.string().default(""), REDACT_EXTRA_KEYS: z.string().default(""),
  INPUT_MAX_BODY_BYTES: coercedInt(262144), INPUT_MAX_MESSAGES: coercedInt(200), INPUT_MAX_ATTACHMENT_BYTES: coercedInt(10485760), INPUT_MAX_TOOLS_PER_REQUEST: coercedInt(20),
  API_KEY_HASH_SECRET: optionalMinString(16), API_KEY_PREFIX: z.string().default("isa_live"), API_KEY_DEFAULT_TTL: coercedInt(2592000), API_KEY_MAX_TTL: coercedInt(31536000), API_KEY_ROTATION_GRACE_SECONDS: coercedInt(300), API_KEY_RATE_LIMIT_DEFAULT: coercedInt(100),
  GENESIS_MAX_TEST_FILES: coercedInt(8),
  ISABELLA_STORAGE_PROVIDER: z.enum(["postgres", "neon", "supabase", "json", "memory"]).default("postgres"),
...
export type EnvVarProvider = "postgres" | "neon" | "supabase" | "stripe" | "gemini" | "openai" | "redis" | "upstash" | "crown" | "bookpi" | "otel" | "oidc" | "vercel" | "self";
export interface EnvVarDescriptor { name: keyof Env; visibility: EnvVarVisibility; required: RuntimeMode[]; forbidden: RuntimeMode[]; provider?: EnvVarProvider; criticality: EnvVarCriticality; rotation?: string; description?: string; }
...
export const ENV_VAR_CATALOG: EnvVarDescriptor[] = [
  ...
  { name: "ALLOW_GUEST_CHAT", visibility: "public", required: [], forbidden: ["staging", "production"], provider: "self", criticality: "HIGH", description: "Chat público permitido solo fuera de staging/production." },
  { name: "CROWN_ENFORCEMENT_MODE", visibility: "public", required: [], forbidden: ["staging", "production"], provider: "crown", criticality: "CRITICAL", description: "dry-run prohibido en producción." },
