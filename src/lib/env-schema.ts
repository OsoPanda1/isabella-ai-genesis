...
export const envSchema = z.object({
  ...
  RATE_LIMIT_DEFAULT_PER_MINUTE: coercedInt(120), RATE_LIMIT_INFERENCE_PER_MINUTE: coercedInt(40), RATE_LIMIT_VOICE_PER_MINUTE: coercedInt(20), GEMINI_API_KEY: optionalString(), LLM_DEFAULT_MODEL: z.string().default("google/gemini-3.8-flash"), LLM_VOICE_MODEL: z.string().default("openai/gpt-4o-mini-tts"), VOICE_API_URL: optionalUrl(), LLM_UPSTREAM_TIMEOUT_MS: coercedInt(8500),
  OTEL_EXPORTER_OTLP_ENDPOINT: optionalUrl(), OTEL_SERVICE_NAME: z.string().default("isabella-ai"), ISABELLA_FEATURE_FLAGS: z.string().default(""), REDACT_EXTRA_KEYS: z.string().default(""), INPUT_MAX_BODY_BYTES: coercedInt(262144), INPUT_MAX_MESSAGES: coercedInt(200), INPUT_MAX_ATTACHMENT_BYTES: coercedInt(10485760), INPUT_MAX_TOOLS_PER_REQUEST: coercedInt(20),
  API_KEY_HASH_SECRET: optionalMinString(16), API_KEY_PREFIX: z.string().default("isa_live"), API_KEY_DEFAULT_TTL: coercedInt(2592000), API_KEY_MAX_TTL: coercedInt(31536000), API_KEY_ROTATION_GRACE_SECONDS: coercedInt(300), API_KEY_RATE_LIMIT_DEFAULT: coercedInt(100), GENESIS_MAX_TEST_FILES: coercedInt(8),
  ISABELLA_STORAGE_PROVIDER: z.enum(["postgres", "neon", "supabase", "json", "memory"]).default("postgres"), DURABLE_JSON_ALLOWED: bool(false), ISABELLA_PAYOUT_CIRCUIT_CERTIFIED: bool(false),
});
...
export function requiredEnvKeys(mode: RuntimeMode): Array<keyof Env> { return ENV_VAR_CATALOG.filter((item) => item.required.includes(mode)).map((item) => item.name); }
