...
describe("smoke de despliegue", () => {
  it("el árbol de rutas expone los endpoints críticos", () => {
    ...
    for (const route of [
      ...
      expect(tree.includes(route), `ruta ausente en árbol: ${route}`).toBe(
        true,
      );
    }
  });
  ...
  it("toolchain canónica: packageManager pnpm 10 y workflows sin pnpm 11", () => {
    ...
    for (const workflow of ["ci.yml", "release.yml", "security.yml"]) {
      ...
      expect(content.includes("version: 11"), `${workflow} usa pnpm 11`).toBe(
        false,
      );
    }
  });
  ...
  it("migraciones RLS de economic_contract existen", () => {
    expect(
      existsSync(
        resolve(
          root,
          "supabase/migrations/20260906090000_economic_contract_rls.sql",
        ),
      ),
    ).toBe(true);
  });
  ...
  it("configuración de deploy Vercel: Nitro + pnpm + output prebuilt", () => {
    ...
    expect(
      viteConfig.includes("nitro/vite"),
      "falta plugin nitro en vite.config",
    ).toBe(true);
    expect(viteConfig.includes('preset: "vercel"'), "falta preset vercel").toBe(
      true,
    );
    ...
    expect(
      lockfile.includes("nitro@3.0.260603-beta"),
      "nitro ausente del lockfile",
    ).toBe(true);
    const pkg = JSON.parse(
      readFileSync(resolve(root, "package.json"), "utf8"),
    ) as {
    ...
    expect(
      ...
      "nitro debe ser dependencia directa",
    ).toBeDefined();
  });
});
