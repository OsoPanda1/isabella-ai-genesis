...
import { embed, embedBytes, cosine, simHash, hammingDistance, compressToLatent } from "@/lib/ncua/embed";
...
describe("ncua:embeddings continuos", () => {
  ...
  it("codifica distinto byte stream en vectores distintos (sin vocabulario/OOV)", () => {
    ...
    expect(cosine(paste, turismo)).toBeLessThan(cosine(paste, embed("mineria de plata", { dim: 192 })));
  });
  ...
  it("simhash: cercanos en el espacio duro tienen distancia Hamming baja", () => {
    ...
    const near = embed("Real del Monte es un pueblo minero de Hidalgo", { dim: 192 });
  ...
  it("comprime bytes a latente por chunks", () => {
    const latent = compressToLatent("compresión continua sin tokens para auditoría", { chunkSize: 8 });
